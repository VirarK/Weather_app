...
async function get_weather(city, country, lat, lon) {
    let api = `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&appid=${key}&lang=fr&units=metric&exclude=minutely,alerts`;
    await fetch(api)
...
async function get_weather_hours(city, country, lat, lon) {
    let api = `https://api.openweathermap.org/data/2.5/onecall/timemachine?lat=${lat}&lon=${lon}&dt=${dt}&appid=${key}&lang=fr&units=metric`;
    await fetch(api)
        .then(function(response) {
            data = response.json();
            return data;
...
async function fill_place(city, country, lat, lon) {
    let api = `https://api.openweathermap.org/geo/1.0/reverse?lat=${lat}&lon=${lon}&limit=2&appid=${key}`;
    await fetch(api)
        .then(function(response) {
            let data = response.json();
            return data;
